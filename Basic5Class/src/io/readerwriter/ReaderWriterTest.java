			...
			if( evt == bSave){
				...
				if( returnValue == JFileChooser.APPROVE_OPTION )
				{
					...
					try{
						...
						FileWriter fw = new FileWriter(f);//사용자가 선택한 파일로 쓰겠다.
						fw.write(content);
						fw.close();
			...
			// "화일읽기" 버튼이 눌렸을 때 
				...
				if( returnValue == JFileChooser.APPROVE_OPTION )
				{
					...
					try {
						...
						FileReader fr = new FileReader(f);//사용자가 선택한 파일로 쓰겠다.
						char[]data = new char[10 * 1024];
						fr.read(data); // 인자에 빈 배열을 넣으면 읽은 것을 data 배열에 넣어준다. 
